# The `ahk` command line interface. Runs CoffeeScript code in an AutoHotKey-like environment 
# Maybe have an interactive REPL.
# Maybe support compiling to an exe.
...
BANNER = '''
	Usage: ahk [options] path/to/script.coffee -- [args]

	If called without options, `ahk` will run your script in a globally polluted environment.
'''

# The list of all the valid option flags that `ahk` knows how to handle.
SWITCHES = [
	['-c', '--compile',         'compile to bytecode and save as .exe']
	['-i', '--interactive',     'run an interactive CoffeeScript + AutoHotKey REPL']
	...
	['-j', '--javascript',      'treat stdio as javascript']
...
version = ->
	printLine "AHK version #{ahk.VERSION}"
